...
|----------|-------------|--------------|----------|
| Device | AVD | Restart Host | Restart this AVD session host and return it to service |
|  |  | Toggle Drain Mode | Enable or disable drain mode on this AVD session host |
|  | General | Assign Groups By Template | Add this device to a predefined set of groups |
...
| Group | Devices | Check Updatable Assets | Check Windows Update for Business enrollment of this group's devices |
|  |  | Unenroll Updatable Assets (Scheduled) | Unenroll this group's devices from Windows Update for Business |
|  | General | Add Or Remove Nested Group | Add a nested group to this group or remove it |
...
|  | Mail | Enable Or Disable External Mail | Allow or block external senders for this Microsoft 365 group |
|  |  | Show Or Hide In Address Book | Show or hide this group in the address book |
|  | Teams | Archive Team | Archive the team of this group |
| Organization | Applications | Add Application Registration | Create an application registration in Entra ID |
...
|  | General | Add Devices Of Users To Group (Scheduled) | Add the devices of a user group's members to a device group |
...
|  | Mail | Add Or Remove Email Address | Add an email address to this user's mailbox or remove one |
...
|  | Security | Confirm Or Dismiss Risky User | Confirm this user as compromised or dismiss the risk |
...
|  | Userinfo | Rename User | Change this user's sign-in name (UPN) and mailbox alias |
