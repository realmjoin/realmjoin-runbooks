...
### Permission notes
Azure Storage Account: 'Storage Blob Data Contributor' role for the Automation Account's managed identity on the target storage account (only required when CreateDownloadLink is used)
